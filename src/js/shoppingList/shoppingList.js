...
import { fetchBook, fetchTopBooks } from '../booksAPI/booksApi';
let items = [];
let itemsPerPage = 3;
...
async function fetchBooks() {
    try {
        ...
        const bookPromises = books.map((book) => fetchBook(book.books[0]._id));
...
function markupItems(pageItems) {
    const markup = pageItems.map(({ list_name, book_image }) => {
        return `
